...
class BERT4Rec(SequentialRecommender):
    def __init__(self, config, dataset):
        ...
        # 使用标准的字段名，BERT4Rec通过掩码训练
        # 对于BERT4Rec，使用标准的序列字段进行掩码训练
        self.MASK_ITEM_SEQ = self.ITEM_SEQ  # 使用标准的item序列字段
        self.POS_ITEMS = self.POS_ITEM_ID   # 使用标准的正样本字段
        self.NEG_ITEMS = self.NEG_ITEM_ID   # 使用标准的负样本字段
    ...
    def get_attention_mask(self, item_seq, bidirectional=False):
        """Generate attention mask for multi-head attention."""
    ...
    def calculate_loss(self, interaction):
        ...
        if self.loss_type == "CE":
            ...
            logits = torch.matmul(seq_output, test_item_emb.transpose(0, 1)) + self.output_bias  # [B, item_num]
            ...
            # BPR损失
            if self.NEG_ITEMS in interaction:
                neg_items = interaction[self.NEG_ITEMS]
            else:
                # 如果没有负样本，随机采样
                neg_items = torch.randint(1, self.n_items, pos_items.shape, device=pos_items.device)
            
            pos_items_emb = self.item_embedding(pos_items)  # [B, H]
            neg_items_emb = self.item_embedding(neg_items)  # [B, H]
            pos_score = torch.sum(seq_output * pos_items_emb, dim=-1) + self.output_bias[pos_items]  # [B]
            neg_score = torch.sum(seq_output * neg_items_emb, dim=-1) + self.output_bias[neg_items]  # [B]
